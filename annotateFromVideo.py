...
def grabWithImg(mask2, last):
    ...
    mask[(mask2 == 2) | (previousMask == 2)] = 2  # 1つ前のマスクで背景 らしかった 領域を、次のマスクでも同等に扱う
...
def draw_line(event,x,y,flags,param):
    ...
    elif event == cv2.EVENT_MOUSEMOVE and draw_line.flag == True:
...
def trackContour(img_bin):  
    ...
    while True:
        ...
        for i in range(7): 
            ...
            if (x+a < 0 or x+a >= img_bin.shape[1]) or (y+b < 0 or y+b >= img_bin.shape[0]): # 画像外じゃないか
                continue;
...
def outputResults(videoTime,img_raw,img,filePath,labelName,importantCorners,rateX,rateY,smallWidth,smallHeight):
